...
def list_pm_plans():
    try:
        ...
        # Attach user count per plan name
...
def get_pm_plan(plan_id: str):
    try:
        result = supabase.table("pm_subscription_plans").select("*").eq("id", plan_id).single().execute()
        if not result.data:
            raise HTTPException(status_code=404, detail="Plan not found")
        plan = result.data
        # Users on this plan
        users_result = supabase.table("users").select("id, name, email").ilike("plan", plan.get("name", "")).execute()
...
def get_support(request_id: str):
    try:
        result = supabase.table("support_requests").select("*").eq("id", request_id).single().execute()
        if not result.data:
            raise HTTPException(status_code=404, detail="Support request not found")
        replies = (supabase.table("support_replies").select("*").eq("request_id", request_id).order("created_at").execute()).data or []
        return {**result.data, "replies": replies}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def reply_support(request_id: str, request: CreateSupportReplyRequest):
    try:
        ...
        # Auto set in_progress if currently open
...
def update_pm_faq(faq_id: str, request: UpdateFaqRequest):
    try:
        ...
        result = supabase.table("help_faqs").select("*").eq("id", faq_id).single().execute()
        return result.data
    except HTTPException:
        raise
    except Exception as e:
...
def system_health():
    try:
        ...
        alerts = []
        try:
            alerts_result = supabase.table("system_alerts").select("*").eq("resolved", False).order("created_at", desc=True).execute()
