...
async def perform_scan(url: str, plan: str = "free") -> dict:
    """
    Layer 1: URLhaus + PhishTank — always run first, results are final.
    Layer 2: Heuristics — runs when Layer 1 is clean.
             DANGEROUS if score >= 40, SUSPICIOUS if >= 20.
    Layer 3: Trusted domain check — only when verdict is SAFE or SUSPICIOUS.
             If trusted → override to SAFE.
    """
