...
# service role client needed to create/delete auth users
...
def list_users(q: Optional[str] = Query(None)):
    try:
        ...
        # batch fetch profile names so we don't do N+1 queries
...
def get_user(user_id: str):
    try:
        ...
        if user.get("profile_id"):
            pr = fetch_one("user_profiles", id=user["profile_id"])
            user["profile_name"] = pr.get("name") if pr else None
        return user
...
def assign_profile(user_id: str, request: AssignProfileRequest):
    try:
        profile = fetch_one("user_profiles", id=request.profile_id)
        if not profile:
            raise HTTPException(status_code=404, detail="Profile not found.")
        ...
        log_admin_action("Profile Assigned", target_type="user", target_id=user_id, details=f"Profile: {profile['name']}")
        return {"message": f"Profile '{profile['name']}' assigned successfully."}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
